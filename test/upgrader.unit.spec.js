...
describe('Test DID Version Upgrader', function() {
  ...
  it('should throw error if new version is not an upgrade on old version', function() {
    ...
    testCases.forEach(version => {
      assert.throw(
        () => DID
          .builder(didId, [...managementKeys], [...didKeys])
          .upgradeSpecVersion(version),
  ...
  it('should export upgrade data correctly', function() {
    ...
    const signedData = "".concat(EntryType.VersionUpgrade, ENTRY_SCHEMA_V100, signingKeyId, entryData['content'].toString());
    ...
    expect(entryData['content']).to.equalBytes(Buffer.from(JSON.stringify({didMethodVersion: newSpecVersion})));
  });
});