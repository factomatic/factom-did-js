...
describe('Test DID Updater', function() {
    ...
    describe('Test Management Keys Update', function() {
        ...
        it('should not rotate nox-existing management key', function() {
            ...
            assert.sameDeepMembers(
                updater.managementKeys,
                (updater as any)._originalManagementKeys
            );
        });
    ...
    describe('Test DID Keys Update', function() {
        ...
        it('should revoke DID key with a single matching purpose', function() {
            ...
            const updater = did
                .update()
                .revokeDIDKeyPurpose(alias, DIDKeyPurpose.AuthenticationKey);
        ...
        it('should revoke DID key with multiple purpose', function() {
            ...
            testCases.forEach(purpose => {
                ...
                assert.strictEqual((updater as any)._didKeyPurposesToRevoke[alias], purpose);
    ...
    describe('Test Services Update', function() {
        ...
        it('should add service', function() {
            ...
            const updater = did
                .update()
                .addService(alias, type, endpoint, priorityRequirement, customFields);
    ...
    describe('Test Export Update Entry Data', function() {
        ...
        it('should export added keys data correctly', function() {
            ...
            const updateEntryData = did
                ...
                .addManagementKey(
                    secondNewMgmtKeyAlias,
                    secondNewMgmtKeyPriority,
                    secondNewMgmtKeyType,
                    secondNewMgmtKeyController,
                    secondNewMgmtKeyPriorityRequirement
                )
                .addDIDKey(
                    newDIDKeyAlias,
                    newDIDKeyPurpose,
            ...
            const signedData = ''.concat(
                ...
                ENTRY_SCHEMA_V100,
                signingKeyId,
                updateEntryData['content'].toString()
            );
            ...
            const _managementKeys = added['managementKey'];
            const _didKeys = added['didKey'];
            const _services = added['service'];

            assert.strictEqual(_managementKeys.length, 2);
            assert.strictEqual(_didKeys.length, 1);
            assert.strictEqual(_services.length, 1);

            const firstAddedMgmtKey = _managementKeys[0];
            ...
            assert.strictEqual(firstAddedMgmtKey.publicKeyBase58, did.managementKeys[3].publicKey);
            assert.isUndefined(firstAddedMgmtKey.priorityRequirement);

            const secondAddedMgmtKey = _managementKeys[1];
            ...
            assert.strictEqual(secondAddedMgmtKey.publicKeyPem, did.managementKeys[4].publicKey);
            assert.strictEqual(
                secondAddedMgmtKey.priorityRequirement,
                secondNewMgmtKeyPriorityRequirement
            );

            const addedDIDKey = _didKeys[0];
            ...
            assert.strictEqual(addedDIDKey.publicKeyBase58, did.didKeys[3].publicKey);
            assert.strictEqual(addedDIDKey.priorityRequirement, newDIDPriorityRequirement);

            const addedService = _services[0];
        ...
        it('should export revoked keys data correctly', function() {
            ...
            expect(extIds[0]).to.equalBytes(Buffer.from(EntryType.Update));
            expect(extIds[1]).to.equalBytes(Buffer.from(ENTRY_SCHEMA_V100));
            expect(extIds[2]).to.equalBytes(Buffer.from(did.managementKeys[0].fullId(didId)));
        ...
        it('should export update entry data correctly', function() {
            ...
            const signingKey = did.managementKeys[0].fullId(didId);
            const updateEntryData = did
                .update()
                .revokeManagementKey('my-first-mgmt-key')
