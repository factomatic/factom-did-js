...
describe('Test DID Keys', function() {
  it('should add DID keys', function() {
    ...
    const did = DID.builder()
      ...
      .didKey(secondDIDKeyAlias, secondDIDKeyPurpose, secondDIDKeyType, secondDIDKeyController, secondDIDKeyPriorityRequirement)
    ...
    assert.strictEqual(secondGeneratedDIDKey.priorityRequirement, secondDIDKeyPriorityRequirement);
    assert.isString(firstGeneratedDIDKey.publicKey);
    assert.isString(firstGeneratedDIDKey.privateKey);
  ...
  it('should throw error if purpose is invalid', function() {
    ...
    assert.throw(
      () => builder.didKey('did-key-1', [DIDKeyPurpose.AuthenticationKey, DIDKeyPurpose.AuthenticationKey]),
    ...
    assert.throw(
      () => builder.didKey('did-key-1', [DIDKeyPurpose.AuthenticationKey, 'invalid-purpose-type']),
  ...
  it('should throw error if controller is invalid', function() {
    ...
    testCases.forEach((controller, index) => {
      assert.throw(
        () => builder.didKey(`did-key-${index}`, DIDKeyPurpose.PublicKey, undefined, controller),
  ...
  it('should throw error if priorityRequired is invalid', function() {
    ...
    testCases.forEach((priorityRequirement, index) => {
      assert.throw(
        () => builder.didKey(`did-key-${index}`, DIDKeyPurpose.PublicKey, undefined, undefined, priorityRequirement),
